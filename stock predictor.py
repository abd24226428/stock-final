...
def load_stock_data(ticker):
    if ticker == "SSNLF":
        st.warning("Samsung (SSNLF) data is limited on Yahoo Finance.")
        return pd.DataFrame()  # Placeholder for SSNLF (no data available)
...
# Display data
st.subheader(f"{ticker} Stock Data (Last 5 Records)")
if df.empty:
...
if not df.empty:
    ...
    if st.button("Train Model"):
        with st.spinner("Training in progress..."):
            ...
            st.session_state["lstm_model"] = model  # Save model in session state
